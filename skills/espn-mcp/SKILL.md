...
- **Step 3: Analyze Injury & Depth Impacts**: Cross-reference `get_team_depth_chart(sport=..., league=..., team_id=...)` with active injury designations in `get_team_roster(sport=..., league=..., team_id=...)` to determine if key starters (e.g., QB1, Ace Pitcher) are ruled out.
...
- **Live Win Probability**: Call `get_game_summary(sport=..., league=..., event_id=...)` to track the live `winprobability` curve across game progression and evaluate in-game swings.
- **Top Performers**: Call `get_player_stats(sport=..., league=..., event_id=...)` to inspect live individual player boxscores (points, strikeouts, yards, touches).
...
- **Recent Results & Momentum**: Call `get_team_schedule(sport=..., league=..., team_id=..., season=...)` to review past 5 games, opponent quality, and margin of victory.
...
- **Career & Season Splits**: Call `get_athlete_overview(sport=..., league=..., athlete_id=...)` to fetch statistical splits (home vs. away, turf vs. grass, vs. opponent).
